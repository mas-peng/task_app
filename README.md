...
アプリケーションの基本的な操作は以下の通りです。

1.  **タスクの表示:**
    アプリケーションにアクセスすると、保存されているタスクが一覧で表示されます。初期状態ではリストは空です。

2.  **タスクの作成:**
    -   ページ上部の入力ボックスに、作成したいタスクのタイトルを入力します。
    -   「Add Task」ボタンをクリックします。
    -   入力したタスクがリストに追加され、画面が自動で更新されます。タスクのステータスは、デフォルトで「Pending」となります。
